...
class BaseMethods:
    ...
    def randomise(self, _, length=5, *_args):
        faker = Faker()
        self.parent.isopen = choice((True, False))
        self.parent.mode = choice(("random", "priority"))
        self.parent.mthds = (
            JDMethods(self.parent) if self.parent.mode == "random" else JBMethods(self.parent)
        )
        self.parent.current_pick = (
            {
                "user": f"user {randint(1, 6)}",
                "entry": " ".join(faker.words(randint(4, 10))),
            },
        )
        self.parent.entries = TupleList(
            *(
                (f"example user {i}", " ".join(faker.words(randint(4, 10))))
                for i in range(1, length + 1)
            )
        )
        self.parent.picked = TupleList(
            *(
                (f"example user {i}", " ".join(faker.words(randint(4, 10))))
                for i in range(randint(1, length))
            )
        )
        return f"Queue has been randomised with {length} entries"
...
class JDMethods(BaseMethods):
    ...
    def pickentry(self, _, selection=None, /, *_args):
...
class SongQueue:
    ...
    def __getattr__(self, attr):
        return getattr(self.mthds, attr)
    ...
    def as_ui_state(self):
        return QueueState(
            self.isopen,
            QueueMode.REQUEST if self.mode == "random" else QueueMode.USER,
            Request(self.current.get("user"), self.current.get("entry")),
            [Request(*i) for i in self.entries.data],
            [Request(*i) for i in self.picked.data],
        )
